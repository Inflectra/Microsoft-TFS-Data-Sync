...
namespace Inflectra.SpiraTest.PlugIns.MsTfsDataSync
{
    ...
    public class DataSync : IServicePlugIn
    {
        ...
        private const string TFS_FIELD_RANK = "Rank";
        private const string TFS_FIELD_TRIAGE = "Triage";
        private const string TFS_FIELD_DISCIPLINE = "Discipline";
        ...
        public ServiceReturnType Execute(Nullable<DateTime> lastSyncDate, DateTime serverDateTime)
        {
            ...
            try
            {
                ...
                //Create a new TFS 2010 project collection instance and WorkItemStore instance
                //Note: This requires that the URI includes the collection name not just the server name
                WorkItemStore workItemStore = null;
                Uri tfsUri = new Uri(this.connectionString);
                TfsTeamProjectCollection tfsTeamProjectCollection = new TfsTeamProjectCollection(tfsUri, credentials);
                ...
                catch (Exception exception)
                {
                    ...
                    {
                        eventLog.WriteEntry("Inner Exception=" + exception.InnerException.Message + ": " + exception.InnerException.StackTrace);
                    }
                    return ServiceReturnType.Error;
                }
                ...
                foreach (SpiraImportExport.RemoteDataMapping projectMapping in projectMappings)
                {
                                ...
                                if (FindMappingByInternalId(projectId, incidentId, incidentMappings) == null)
                                {
                                    ...
                                    if (remoteIncident.DetectedReleaseId.HasValue)
                                    {
                                        ...
                                        if (dataMapping == null)
                                        {
                                            ...
                                            Node newIterationNode = AddNewTfsIteration(tfsTeamProjectCollection, ref workItemStore, ref project, remoteIncident.DetectedReleaseVersionNumber);
                                    ...
                                    //See if this work item type supports the rich-text 'steps to reproduce' field
                                    if (workItemType.FieldDefinitions.Contains(TFS_FIELD_STEPS_TO_REPRODUCE))
                                    {
                                        workItem[TFS_FIELD_STEPS_TO_REPRODUCE] = MakeXmlSafe(remoteIncident.Description);
                                    }
                                    //The description field only supports plain text
                                    workItem.Description = HtmlRenderAsPlainText(remoteIncident.Description);
                                    ...
                                    {
                                        dataMapping = FindMappingByInternalId(remoteIncident.OpenerId, userMappings);
                                        if (dataMapping == null)
                                        {
                                        ...
                                        if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_TEXT)
                                        {
                                            ...
                                            if (!String.IsNullOrEmpty(customPropertyValue))
                                            {
                                                ...
                                                if (incidentCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    {
                                                        workItem[TFS_FIELD_RANK] = customPropertyValue;
                                                    }
                                                    else if (workItem.Type.FieldDefinitions.Contains(externalCustomField))
                                                    {
                                                ...
                                                if (incidentCustomPropertyValueMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    if (customPropertyValueMapping != null)
                                                    {
                                                        ...
                                                        else if (externalCustomField == TFS_SPECIAL_FIELD_TRIAGE)
                                                        {
                                                            ...
                                                            if (workItem.Fields.Contains(TFS_FIELD_TRIAGE))
                                                            {
                                                                workItem[TFS_FIELD_TRIAGE] = externalCustomFieldValue;
                                                            }
                                                        }
                                                        else if (workItem.Type.FieldDefinitions.Contains(externalCustomField))
                                                        {
                                    ...
                                    if (ValidateItem(workItem, messages))
                                    {
                                        ...
                                        {
                                            workItem.History = remoteResolution.Resolution;
                                            workItem.Save();
                                        }
                                    }
                                    else
                                ...
                                if (FindMappingByInternalId(projectId, taskId, taskMappings) == null && remoteTask.Name.Trim() != NEW_TASK_NAME_TO_IGNORE)
                                {
                                    ...
                                    if (remoteTask.ReleaseId.HasValue)
                                    {
                                        ...
                                        if (dataMapping == null)
                                        {
                                            //We can't find the matching item so need to create a new iteration in TFS and add to mappings
                                            LogTraceEvent(eventLog, "Adding new iteration in TFS for release " + detectedReleaseId + "\n", EventLogEntryType.Information);
                                            Node newIterationNode = AddNewTfsIteration(tfsTeamProjectCollection, ref workItemStore, ref project, remoteTask.ReleaseVersionNumber);
                                            ...
                                            if (customPropertyValue.HasValue && taskCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                            {
                                                ...
                                                if (taskCustomPropertyValueMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    if (customPropertyValueMapping != null)
                                                    {
                                                        ...
                                                        {
                                                            //Now set the value of the work item's triage status
                                                            workItem[TFS_FIELD_DISCIPLINE] = externalCustomFieldValue;
                                                        }
                                                        else if (workItem.Type.FieldDefinitions.Contains(externalCustomField))
                                                        {
                                                            //This needs to be added to the list of TFS custom properties
                    ...
                    foreach (WorkItem workItem in workItemCollection)
                    {
                            ...
                            if (workItem.Type == project.WorkItemTypes["Task"])
                            {
                                if (String.IsNullOrEmpty(this.syncOnlyType) || this.syncOnlyType.ToLowerInvariant() == "tasks")
                                {
                                    ...
                                    if (taskMapping == null)
                                    {
                                        ...
                                        bool changesMade = false;   //Not used
                                        foreach (SpiraImportExport.RemoteCustomProperty customProperty in taskProjectCustomProperties)
                                        {
                                            ...
                                            if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_TEXT)
                                            {
                                                if (taskCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    if (externalCustomField == TFS_SPECIAL_FIELD_RANK)
                                                    {
                                                        ...
                                                        changesMade = SetCustomPropertyTextValue(remoteTask, customProperty.CustomPropertyName, (string)workItem[TFS_FIELD_RANK], changesMade);
                                                    }
                                                    else if (workItem.Type.FieldDefinitions.Contains(externalCustomField) && workItem[externalCustomField] is String)
                                                    {
                                                        //Now we need to set the value on the SpiraTest task
                                                        changesMade = SetCustomPropertyTextValue(remoteTask, customProperty.CustomPropertyName, (string)workItem[externalCustomField], changesMade);
                                                    }
                                                }
                                            }
                                            ...
                                            if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_LIST)
                                            {
                                                if (taskCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    else if (externalCustomField == TFS_SPECIAL_FIELD_DISCIPLINE && workItem[TFS_FIELD_DISCIPLINE] != null)
                                                    {
                                                        //Now we need to set the value on the SpiraTest task
                                                        SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = taskCustomPropertyValueMappingList[customProperty.CustomPropertyId];
                                                        SpiraImportExport.RemoteDataMapping customPropertyValueMapping = FindMappingByExternalKey(projectId, (string)workItem[TFS_FIELD_DISCIPLINE], customPropertyValueMappings, false);
                                                        if (customPropertyValueMapping != null)
                                                        {
                                                            changesMade = SetCustomPropertyListValue(remoteTask, customProperty.CustomPropertyName, customPropertyValueMapping.InternalId, changesMade);
                                                        }
                                                    ...
                                                    else if (workItem.Type.FieldDefinitions.Contains(externalCustomField))
                                                    {
                                                        //Now we need to set the value on the SpiraTest task
                                                        SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = taskCustomPropertyValueMappingList[customProperty.CustomPropertyId];
                                                        SpiraImportExport.RemoteDataMapping customPropertyValueMapping = FindMappingByExternalKey(projectId, workItem[externalCustomField].ToString(), customPropertyValueMappings, false);
                                                        if (customPropertyValueMapping != null)
                                                        {
                                                            changesMade = SetCustomPropertyListValue(remoteTask, customProperty.CustomPropertyName, customPropertyValueMapping.InternalId, changesMade);
                                                        }
                            ...
                            else
                            {
                                if (String.IsNullOrEmpty(this.syncOnlyType) || this.syncOnlyType.ToLowerInvariant() == "incidents")
                                {
                                    ...
                                    if (incidentMapping == null)
                                    {
                                        ...
                                        {
                                            remoteIncident.Description = MakeXmlSafe(workItem[TFS_FIELD_STEPS_TO_REPRODUCE]);
                                        }
                                        else
                                        {
                                            if (String.IsNullOrEmpty(workItem.Description))
                                            ...
                                            if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_TEXT)
                                            {
                                                if (incidentCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    if (externalCustomField == TFS_SPECIAL_FIELD_RANK)
                                                    {
                                                        ...
                                                        changesMade = SetCustomPropertyTextValue(remoteIncident, customProperty.CustomPropertyName, (string)workItem[TFS_FIELD_RANK], changesMade);
                                                    }
                                                    else if (workItem.Type.FieldDefinitions.Contains(externalCustomField) && workItem[externalCustomField] is String)
                                                    {
                                                        //Now we need to set the value on the SpiraTest incident
                                                        changesMade = SetCustomPropertyTextValue(remoteIncident, customProperty.CustomPropertyName, (string)workItem[externalCustomField], changesMade);
                                                    }
                                                }
                                            }
                                            ...
                                            if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_LIST)
                                            {
                                                if (incidentCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                {
                                                    ...
                                                    else if (externalCustomField == TFS_SPECIAL_FIELD_TRIAGE)
                                                    {
                                                        //Now we need to set the value on the SpiraTest incident
                                                        SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = incidentCustomPropertyValueMappingList[customProperty.CustomPropertyId];
                                                        SpiraImportExport.RemoteDataMapping customPropertyValueMapping = FindMappingByExternalKey(projectId, (string)workItem[TFS_FIELD_TRIAGE], customPropertyValueMappings, false);
                                                        if (customPropertyValueMapping != null)
                                                        {
                                                            changesMade = SetCustomPropertyListValue(remoteIncident, customProperty.CustomPropertyName, customPropertyValueMapping.InternalId, changesMade);
                                                        }
                                        ...
                                        {
                                            dataMapping = FindMappingByExternalKey((string)workItem[CoreField.CreatedBy], userMappings);
                                            if (dataMapping == null)
                                            {
                                                //We can't find the matching user so log and ignore
                                                eventLog.WriteEntry("Unable to locate mapping entry for TFS user " + (string)workItem[CoreField.CreatedBy] + " so using the synchronization user", EventLogEntryType.Error);
                                            }
                                            else
                                            {
                                        ...
                                        //Finally add the SpiraTeam incident ID to TFS if appropriate
                                        if (!String.IsNullOrEmpty(this.artifactIdTfsField) && workItem.Type.FieldDefinitions.Contains(this.artifactIdTfsField))
                                        {
                                            workItem[this.artifactIdTfsField] = ARTIFACT_TYPE_PREFIX_INCIDENT + incidentId;
                                            workItem.Save();
                                        }
                        ...
                        catch (Exception exception)
                        {
                            ...
                            eventLog.WriteEntry("Error adding TFS work item to " + productName + " - error message = " + exception.Message, EventLogEntryType.Error);
                        }
                    }
                                    ...
                                    if (updateMode == "Spira=Newer")
                                    {
                                        ...
                                        if (remoteIncident.ResolvedReleaseId.HasValue)
                                        {
                                            ...
                                            if (dataMapping == null)
                                            {
                                                //We can't find the matching item so need to create a new iteration in TFS and add to mappings
                                                LogTraceEvent(eventLog, "Adding new iteration in TFS for release " + detectedReleaseId + "\n", EventLogEntryType.Information);
                                                Node newIterationNode = AddNewTfsIteration(tfsTeamProjectCollection, ref workItemStore, ref project, remoteIncident.ResolvedReleaseVersionNumber);
                                        ...
                                        if (workItem.Type.FieldDefinitions.Contains(TFS_FIELD_STEPS_TO_REPRODUCE) && !String.IsNullOrEmpty(workItem[TFS_FIELD_STEPS_TO_REPRODUCE].ToString()))
                                        {
                                            string stepsToReproduce = MakeXmlSafe(workItem[TFS_FIELD_STEPS_TO_REPRODUCE]);
                                            if (stepsToReproduce != MakeXmlSafe(remoteIncident.Description))
                                            {
                                                workItem[TFS_FIELD_STEPS_TO_REPRODUCE] = MakeXmlSafe(remoteIncident.Description);
                                                changesMade = true;
                                            }
                                            ...
                                            if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_TEXT)
                                            {
                                                ...
                                                if (!String.IsNullOrEmpty(customPropertyValue))
                                                {
                                                    ...
                                                    if (incidentCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                    {
                                                        ...
                                                        if (externalCustomField == TFS_SPECIAL_FIELD_RANK)
                                                        {
                                                            if (workItem[TFS_FIELD_RANK].ToString() != customPropertyValue)
                                                            {
                                                                workItem[TFS_FIELD_RANK] = customPropertyValue;
                                                                changesMade = true;
                                                            }
                                                    ...
                                                    if (incidentCustomPropertyValueMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                    {
                                                        ...
                                                        if (customPropertyValueMapping != null)
                                                        {
                                                            ...
                                                            else if (externalCustomField == TFS_SPECIAL_FIELD_TRIAGE)
                                                            {
                                                                ...
                                                                if (workItem.Fields.Contains(TFS_FIELD_TRIAGE))
                                                                {
                                                                    if (workItem[TFS_FIELD_TRIAGE].ToString() != externalCustomFieldValue)
                                                                    {
                                                                        workItem[TFS_FIELD_TRIAGE] = externalCustomFieldValue;
                                                                        changesMade = true;
                                                                    }
                                        ...
                                        RemoteIncidentResolution[] remoteResolutions = spiraImportExport.Incident_RetrieveResolutions(incidentId);
                                        LogTraceEvent(eventLog, "Checking to see if we need to add " + productName + " resolutions to TFS for incident IN" + remoteIncident.IncidentId + ". There are currently " + remoteResolutions.Length + " comments in " + productName + " and " + workItem.Revisions.Count + " revisions in TFS", EventLogEntryType.Information);
                                        foreach (RemoteIncidentResolution remoteResolution in remoteResolutions)
                                        {
                                            ...
                                            foreach (Revision revision in workItem.Revisions)
                                            {
                                                ...
                                                if (revision.Fields[CoreField.History].Value != null && revision.Fields[CoreField.History].Value.ToString() != "")
                                                {
                                                    ...
                                                    string resolutionDescription = remoteResolution.Resolution;
                                            ...
                                            {
                                                LogTraceEvent(eventLog, "Adding new resolution '" + remoteResolution.Resolution + " to TFS (no existing comments)", EventLogEntryType.Information);
                                                workItem.History = remoteResolution.Resolution;
                                                changesMade = true;
                                            }
                                        }
                                    ...
                                    if (updateMode == "TFS=Newer")
                                    {
                                        ...
                                        try
                                        {
                                            if (workItem.Fields.Contains(TFS_FIELD_PRIORITY) && workItem.Fields[TFS_FIELD_PRIORITY].IsValid)
                                            {
                                                ...
                                                {
                                                    dataMapping = FindMappingByExternalKey(projectId, workItem[TFS_FIELD_PRIORITY].ToString(), incidentPriorityMappings, true);
                                                    if (dataMapping == null)
                                                    {
                                                        //We can't find the matching item so log and just don't set the priority
                                                ...
                                                if (customProperty.CustomPropertyTypeId == CUSTOM_PROPERTY_TYPE_TEXT)
                                                {
                                                    ...
                                                    if (!String.IsNullOrEmpty(customPropertyValue))
                                                    {
                                                        ...
                                                        if (taskCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                        {
                                                            ...
                                                            if (externalCustomField == TFS_SPECIAL_FIELD_RANK)
                                                            {
                                                                if (workItem.Fields.Contains(TFS_FIELD_RANK))
                                                                {
                                                                    if (workItem[TFS_FIELD_RANK].ToString() != customPropertyValue)
                                                                    {
                                                                        workItem[TFS_FIELD_RANK] = customPropertyValue;
                                                    ...
                                                    if (customPropertyValue.HasValue && taskCustomPropertyMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                    {
                                                        ...
                                                        if (taskCustomPropertyValueMappingList.ContainsKey(customProperty.CustomPropertyId))
                                                        {
                                                            ...
                                                            if (customPropertyValueMapping != null)
                                                            {
                                                                ...
                                                                {
                                                                    //Now set the value of the work item's triage status
                                                                    if (workItem.Fields.Contains(TFS_FIELD_DISCIPLINE))
                                                                    {
                                                                        if (workItem[TFS_FIELD_DISCIPLINE].ToString() != externalCustomFieldValue)
                                                                        {
                                                                            workItem[TFS_FIELD_DISCIPLINE] = externalCustomFieldValue;
                                                                            changesMade = true;
                                                                        }
